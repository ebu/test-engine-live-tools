...
var config = {
  ...
  transcoding: {
    ffmpeg_opts: ['-re', '-i', '-', '-threads', '0', '-y'],
    hls_opts: ['-hls_time', '10', '-hls_list_size', '5'],
    video_opts: ['-vcodec', 'libx264', '-vprofile', 'baseline', '-preset', 'veryfast', '-level', '31', '-keyint_min', '25', '-g', '25', '-r', '25', '-sc_threshold', '0'],
...
config.representations = {
  ...
  video_low:      config.transcoding.video_opts.concat(['-vb', '512k', '-bufsize', '1024k', '-maxrate', '512k', '-s', '480x270', '-an']).concat(config.transcoding.hls_opts),
  video_high:     config.transcoding.video_opts.concat(['-vb', '2048k', '-bufsize', '4096k', '-maxrate', '2048k', '-s', '768x432', '-an']).concat(config.transcoding.hls_opts),
  video_hd:       config.transcoding.video_opts.concat(['-vb', '4096k', '-bufsize', '8192k', '-maxrate', '4096k', '-s', '1280x720', '-an']).concat(config.transcoding.hls_opts)
}
