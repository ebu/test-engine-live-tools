...
config.representations = {
  video_hd:       config.transcoding.video_opts.concat(['-vb', '4096k', '-s', '1280x720', '-an']).concat(config.transcoding.hls_opts),
  video_veryhigh: config.transcoding.video_opts.concat(['-vb', '3072k', '-s', '960x540', '-an']).concat(config.transcoding.hls_opts),
  video_high:     config.transcoding.video_opts.concat(['-vb', '2048k', '-s', '768x432', '-an']).concat(config.transcoding.hls_opts),
  video_med:      config.transcoding.video_opts.concat(['-vb', '1024k', '-s', '640x360', '-an']).concat(config.transcoding.hls_opts),
  video_low:      config.transcoding.video_opts.concat(['-vb', '512k', '-s', '480x270', '-an']).concat(config.transcoding.hls_opts),
  audio:     config.transcoding.audio_opts.concat(['-ab', '128k']).concat(config.transcoding.hls_opts)
}

config.packaging = {
  mp4box_opts: ['-dash-ctx', path.join(config.output_dir, 'dash-live.txt'), '-dash', '10000', '-rap', '-ast-offset', '11', '-no-frags-default',
