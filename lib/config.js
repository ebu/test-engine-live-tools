...
config.representations = {
  video_hd:       config.transcoding.video_opts.concat(['-vb', '4000k', '-bufsize', '8000k', '-maxrate', '4000k', '-s', '1280x720', '-an']).concat(config.transcoding.hls_opts),
  //video_veryhigh: config.transcoding.video_opts.concat(['-vb', '3072k', '-s', '960x540', '-an']).concat(config.transcoding.hls_opts),
  video_high:     config.transcoding.video_opts.concat(['-vb', '2000k', '-bufsize', '4000k', '-maxrate', '2000k', '-s', '768x432', '-an']).concat(config.transcoding.hls_opts),
  //video_med:      config.transcoding.video_opts.concat(['-vb', '1024k', '-s', '640x360', '-an']).concat(config.transcoding.hls_opts),
  video_low:      config.transcoding.video_opts.concat(['-vb', '500k', '-bufsize', '1000k', '-maxrate', '500k', '-s', '480x270', '-an']).concat(config.transcoding.hls_opts),
