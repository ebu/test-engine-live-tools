...
function updateSegments(filename) {
  ...
  if (availableSegments.length > representationCount) {
    // Get current representation
    var representationKey = config.extractRepresentationKey(filename);
...
function updateMPD() {
  if (packagedSegments.length % representationCount == 0 && packagedSegments.length > 0) {
    var segmentsToBeProcessed = packagedSegments.splice(0, representationCount);
